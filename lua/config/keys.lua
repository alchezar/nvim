...
map('n', '<D-e>', function() builtin.buffers({ sort_mru = true, ignore_current_buffer = true }) end, { desc = 'Open buffers (MRU)' })
map('n', '<C-p>', builtin.find_files, { desc = 'Find files' })
map('n', '<leader>ff', builtin.find_files, { desc = 'Find files (fuzzy)' })
map('n', '<leader>fg', function() builtin.live_grep({ additional_args = function() return { '--fixed-strings' } end }) end, { desc = 'Live grep (literal)' }) -- no regex so (), [],... need no escaping .
...
map('n', '<leader>cf', function() utils.yank_to_clipboard(vim.fn.expand('%:t') .. ':' .. vim.fn.line('.')) end, { desc = 'Copy filename:line' })
...
map('n', '<leader>gr', function() require('gitsigns').reset_hunk(); vim.cmd('noautocmd write') end, { desc = 'Reset hunk under cursor and save (no autoformat)' })
